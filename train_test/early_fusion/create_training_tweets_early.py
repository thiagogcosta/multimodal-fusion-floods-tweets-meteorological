...
from preprocessing_tweets_early_fusion import Preprocessing
import numpy

#------------------PRE-PROCESSING - FastText / Word2Vec---------------------
def preprocess_tweets_fasttext_word2vec(tweets_early_fusion, dict_hashtag, kw, con_mongo, my_database, type_name):
    ...
    for k in type_name:
        ...
        result_skip50 = Preprocessing(tweets_early_fusion,kw, model_skip50, False, dict_hashtag)

        print('---------------------result_skip50---------------------')
        print(result_skip50)

        result_skip100 = Preprocessing(tweets_early_fusion,kw, model_skip100, False,dict_hashtag)

        print('---------------------result_skip100---------------------')
        print(result_skip100)

        result_cbow50 = Preprocessing(tweets_early_fusion,kw, model_cbow50, False,dict_hashtag)

        print('---------------------result_cbow50---------------------')
        print(result_cbow50)

        result_cbow100 = Preprocessing(tweets_early_fusion,kw, model_cbow100, False, dict_hashtag)
...
def preprocess_tweets_bow_tfidf(tweets_early_fusion, dict_hashtag, con_mongo, my_database):
    ...
    result = Preprocessing(tweets_early_fusion,kw, model, True, dict_hashtag)
    ...
    for l in name_type:
        ...
        for k in range(len(vec_name)):
            ...
            for i in range(len(model_tweets_embed)):
                ...
                aux_emb = numpy.append(aux_emb, model_tweets_embed.loc[i]['index'])
            ...
            data_model.to_csv(local_model_result+'socialflood_modelTraining_'+vec_name[k]+'_tweets_early_fusion.csv')
            print('-----------------------------------------------')
...
def create_training_data_bow_tfidf(con_mongo, my_database):
    ...
    for l in name_type:
        ...
        data_model.to_csv(local_model_result+'socialflood_modelTraining_'+l+'_tweets_early_fusion.csv')
        print('-----------------------------------------------')
...
local_model_result = '/home/thiago-costa/projects/multimodal-fusion-floods-tweets-meteorological/train_test/early_fusion/data/'
...
my_database = con_mongo.create_db('enoesocial-tweets_early_fusion-OK-JOURNAL')
...
tweets_early_fusion = pd.read_csv(local_model + 'THIAGO_NOV2016_NOV2018_judge_CERTO_INSIDE_sp_cluster_FEATURES_modelo_early_fusion.csv' , sep=(','))

tweets_early_fusion = tweets_early_fusion[['index', 'id_str', 'created_at', 
                                           'longitude', 'latitude',
                                           'data', 'hora_complete', 'text', 
...
preprocess_tweets_fasttext_word2vec(tweets_early_fusion, dict_hashtag, kw, con_mongo, my_database, type_name)
...
#------------------PRE-PROCESSING - BOW / TF-IDF---------------------
preprocess_tweets_bow_tfidf(tweets_early_fusion, dict_hashtag, con_mongo, my_database)
