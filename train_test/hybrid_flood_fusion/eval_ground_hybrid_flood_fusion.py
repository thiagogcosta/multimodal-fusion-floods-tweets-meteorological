...
from preprocessing_tweets_hybrid_flood_fusion import Preprocessing
...
#----------------------PRE-PROCESSING - Word2Vec - SkipGram------------------------

def data_transformation_word2vec(ground_truth, key_words, dict_hashtag, con_mongo, my_database):
...
def create_model_training(con_mongo, my_database):
    ...
    for l in name_type:
        ...
        for k in range(len(vec_name)):
            ...
            data_model.to_csv(local_model_result+'socialflood_model_conj_vdd_'+l+'_tweets_hybrid_flood_fusion.csv')

#------------------------------------DATAFRAME hybrid flood fusion 100 DIMENSÕES------------------------------------

def processing_df_hybrid_flood_100(model, ground_truth):
...
# hybrid_flood_fusion folder location
local_model_result = '/home/thiago-costa/projects/multimodal-fusion-floods-tweets-meteorological/train_test/hybrid_flood_fusion/data/'
...
my_database = con_mongo.create_db('enoesocial-tweets_hybrid_flood_fusion-EVAL-OK-JOURNAL')
...
data_transformation_word2vec(ground_truth, key_words, dict_hashtag, con_mongo, my_database)
...
model = pd.read_csv(local_model_result+'socialflood_model_conj_vdd_Word2Vec_tweets_hybrid_flood_fusion.csv')
...
ground_truth_ok = processing_df_hybrid_flood_100(model, ground_truth)

print(ground_truth_ok)

ground_truth_ok.to_csv(local_model_result + 'socialflood_model_conj_vdd_Word2Vec_tweets_hybrid_flood_fusion_CERTO.csv')


#***********Processing***********

ground_truth = pd.read_csv(local_model_result + 'socialflood_model_conj_vdd_Word2Vec_tweets_hybrid_flood_fusion_CERTO.csv')

data = ground_truth.reset_index()
...
scaler = joblib.load(local_model_result + 'modelo_late_fusion_alagamento_scaler.sav')

data_alag = scaler.fit_transform(data_alag)

loaded_model = joblib.load(local_model_result + 'dt_train_modelo_late_fusion_alagamento.sav')

result = loaded_model.predict(data_alag)

print('-----------------------------------alag metrics--------------------------------------')

metrics = precision_recall_fscore_support(data['is_alag'].values, result, average='macro')
...
for i in range(len(result)):
    ...
    data.loc[i, 'predict_alag'] = result[i]
    #print(i)
...
loaded_model = joblib.load(local_model_result + 'dt_train_model_hybrid_flood_fusion.sav')
...
df_result.to_csv(local_model_result + 'resultado_predict_hybrid_alag_fusion.csv')

df_target = pd.DataFrame(data=result.flatten())

df_target.to_csv(local_model_result + 'resultado_target_hybrid_alag_fusion.csv')
