...
import pandas as pd
from ml_unimodal_fusion import ML
import pickle
import time
...
def processing_dataframe(vec_data_50, vec_data_100):
    ...
    for i in vec_data_50:
        df = pd.read_csv(local_unimodal_fusion_model + i +'.csv')
...
def processing_df_unimodal_fusion_50(vec_50_df, unimodal_fusion_data, vec_data_50):
    ...
    for model in vec_50_df:
        ...
        for i in range(len(model)):
            ...
            for j in range(len(unimodal_fusion_data)):
                
                #print(j)
                
                if model.loc[i]['index'] == unimodal_fusion_data.loc[j]['index']:
                    
                    model.loc[i, 'temperatura'] = unimodal_fusion_data.loc[j]['temperatura']
                    model.loc[i, 'temperatura_ponto_orvalho'] = unimodal_fusion_data.loc[j]['temperatura_ponto_orvalho']
                    model.loc[i, 'pressao_atmosferica'] = unimodal_fusion_data.loc[j]['pressao_atmosferica']
                    model.loc[i, 'umidade'] = unimodal_fusion_data.loc[j]['umidade']
                    model.loc[i, 'precipitacao'] = unimodal_fusion_data.loc[j]['precipitacao']
                    model.loc[i, 'related'] = unimodal_fusion_data.loc[j]['related']
                    model.loc[i, 'inside_cluster'] = unimodal_fusion_data.loc[j]['inside_cluster']
                    model.loc[i, 'is_alag'] = unimodal_fusion_data.loc[j]['is_alag']

                    if unimodal_fusion_data.loc[j]['related'] == 1.0 and unimodal_fusion_data.loc[j]['inside_cluster'] == 1.0 and unimodal_fusion_data.loc[j]['is_alag'] == 1.0:
        ...
        model.to_csv(local_unimodal_fusion_model + vec_data_50[count]+ '_CERTO.csv')
...
def processing_df_early_fusion_100(vec_100_df, unimodal_fusion_data, vec_data_100):
    ...
    for model in vec_100_df:
        ...
        model.to_csv(local_unimodal_fusion_model + vec_data_100[count]+ '_CERTO.csv')
...
local_unimodal_fusion_model = '/home/thiago-costa/projects/multimodal-fusion-floods-tweets-meteorological/train_test/unimodal_fusion/data/'

# =============================================================================      

start = time.time()

unimodal_fusion_data = local_model + 'THIAGO_NOV2016_NOV2018_judge_CERTO_INSIDE_sp_cluster_FEATURES_modelo_early_fusion.csv'

unimodal_fusion_data  = pd.read_csv(unimodal_fusion_data)
...
#processing_df_early_fusion_50(vector_dataframes[0], unimodal_fusion_data, vec_data_50)

#-------------------PROCESSING DATAFRAME EARLY FUSION 100 DIMENSIONS-------------------

#processing_df_early_fusion_100(vector_dataframes[1], unimodal_fusion_data, vec_data_100)
...
#models_types = ['socialflood_modelTraining_BOW_tweets_early_fusion_CERTO.csv']  

#------------------DATA SPLITING------------------

data = pd.read_csv(local_unimodal_fusion_model + models_types[0])
...
for types in models_types:
    ...
    data = pd.read_csv(local_unimodal_fusion_model + types)
    ...
    columns = ['algoritmo','acuracia', 'precisao', 'recall', 'f1-score']
    ...
    #filename = local_unimodal_fusion_model+'dt_train_model_unimodal_fusion.sav'
    ...
    #df_result.to_csv(local_unimodal_fusion_model + 'resultado_target_classificadores_unimodal_fusion_BOW.csv')

    #df_metrics.to_csv(local_unimodal_fusion_model + 'resultado_classificadores_unimodal_fusion_BOW.csv')
...
df_results_metrics.to_csv(local_unimodal_fusion_model + 'resultado_classificadores_unimodal_fusion.csv')

#-----------------PREPROCESSING RESULTS-----------------

#data_results = pd.read_csv(local_unimodal_fusion_model + 'resultado_classificadores_early_fusion.csv')

#data_results_ok = preprocessing_results(data_results)

#data_results_ok.to_csv(local_unimodal_fusion_model + 'resultado_classificadores_tweets_2016_2017_describe_CERTO.csv')
