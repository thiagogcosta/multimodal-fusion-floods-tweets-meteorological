...
import pandas as pd
from ml_hybrid_tweet_fusion import ML
import pickle
import time
import joblib
...
def processing_dataframe(vec_data_50, vec_data_100):
    ...
    for i in vec_data_50:
        df = pd.read_csv(local_hybrid_tweet_fusion_model + i +'.csv')
...
#-------------------PROCESSING DATAFRAME HYBRID TWEET FUSION 50 DIMENSIONS-------------------

def processing_df_hybrid_tweet_fusion_50(vec_50_df, hybrid_tweet_fusion, vec_data_50):
    ...
    for model in vec_50_df:
        ...
        for i in range(len(model)):
            ...
            for j in range(len(hybrid_tweet_fusion)):
                
                #print(j)
                
                if model.loc[i]['index'] == hybrid_tweet_fusion.loc[j]['index']:
                    
                    model.loc[i, 'temperatura'] = hybrid_tweet_fusion.loc[j]['temperatura']
                    model.loc[i, 'temperatura_ponto_orvalho'] = hybrid_tweet_fusion.loc[j]['temperatura_ponto_orvalho']
                    model.loc[i, 'pressao_atmosferica'] = hybrid_tweet_fusion.loc[j]['pressao_atmosferica']
                    model.loc[i, 'umidade'] = hybrid_tweet_fusion.loc[j]['umidade']
                    model.loc[i, 'precipitacao'] = hybrid_tweet_fusion.loc[j]['precipitacao']
                    model.loc[i, 'related'] = hybrid_tweet_fusion.loc[j]['related']
                    model.loc[i, 'inside_cluster'] = hybrid_tweet_fusion.loc[j]['inside_cluster']
                    model.loc[i, 'is_alag'] = hybrid_tweet_fusion.loc[j]['is_alag']

                    if hybrid_tweet_fusion.loc[j]['related'] == 1.0 and hybrid_tweet_fusion.loc[j]['inside_cluster'] == 1.0 and hybrid_tweet_fusion.loc[j]['is_alag'] == 1.0:
        ...
        model.to_csv(local_hybrid_tweet_fusion_model + vec_data_50[count]+ '_CERTO.csv')
        
        count += 1
        
        print(model)

#-------------------PROCESSING DATAFRAME HYBRID TWEET FUSION 100 DIMENSIONS-------------------

def processing_df_hybrid_tweet_fusion_100(vec_100_df, hybrid_tweet_fusion, vec_data_100):
    ...
    for model in vec_100_df:
        ...
        model.to_csv(local_hybrid_tweet_fusion_model + vec_data_100[count]+ '_CERTO.csv')
...
local_hybrid_tweet_fusion_model = '/home/thiago-costa/projects/multimodal-fusion-floods-tweets-meteorological/train_test/hybrid_tweet_fusion/data/'
...
processing_df_hybrid_tweet_fusion_50(vector_dataframes[0], early_fusion_data, vec_data_50)

#-------------------PROCESSING DATAFRAME HYBRID TWEET FUSION 100 DIMENSIONS-------------------

processing_df_hybrid_tweet_fusion_100(vector_dataframes[1], early_fusion_data, vec_data_100)
...
models_type = 'socialflood_modelTraining_FastText_skip100_tweets_early_fusion_CERTO.csv'

data = pd.read_csv(local_hybrid_tweet_fusion_model + models_type)

data = data[data['index'] != 8559.0]

rel = data[data['target'] == 0]

not_rel = data[data['target'] == 1]
                                                                                                    
data_not_related = not_rel.sample(n = 449) 

data_related = rel.sample(n = 449) 
...
#-----------------------------PREDICT TWEETS---------------------------------
tweet_model = joblib.load(local_hybrid_tweet_fusion_model + 'rf_train_modelo_late_fusion_tweets.sav')
...
random_forest = aux.RF_classifier(data)

data_train = data_train.sample(n = 898)

naive_bayes = aux.NB_classifier(data_train)

data_train = data_train.sample(n = 898)

decision_tree = aux.DT_classifier(data_train)

data_train = data_train.sample(n = 898)

logistic_regression = aux.RL_classifier(data_train)

data_train = data.sample(n = 898)

svm_class = aux.SVM_classifier(data_train)
...
filename = local_hybrid_tweet_fusion_model+'rf_train_model_hybrid_tweet_fusion.sav'
pickle.dump(random_forest[7], open(filename, 'wb'))

print(df_result)

print(df_metrics)

df_result.to_csv(local_hybrid_tweet_fusion_model + 'resultado_target_classificadores_hybrid_tweet_fusion.csv')

df_metrics.to_csv(local_hybrid_tweet_fusion_model + 'resultado_classificadores_hybrid_tweet_fusion.csv')

#-----------------PREPROCESSING RESULTS-----------------

#data_results = pd.read_csv(local_hybrid_tweet_fusion_model + 'resultado_classificadores_early_fusion.csv')

#data_results_ok = preprocessing_results(data_results)

#data_results_ok.to_csv(local_hybrid_tweet_fusion_model + 'resultado_classificadores_tweets_2016_2017_describe_CERTO.csv')
